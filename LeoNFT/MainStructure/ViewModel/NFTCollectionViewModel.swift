...
class NFTAssetViewModel {
    ...
    func getImageURL() -> String {
        if !(nftAsset.image_url ?? "").isEmpty {
            return nftAsset.image_url!
        } else if !(nftAsset.image_preview_url ?? "").isEmpty {
            return nftAsset.image_preview_url!
        } else if !(nftAsset.image_original_url ?? "").isEmpty {
            return nftAsset.image_original_url!
        } else if !(nftAsset.image_thumbnail_url ?? "").isEmpty {
            return nftAsset.image_thumbnail_url!
        } else {
            return "https://lh3.googleusercontent.com/9WkSu8CP7gZjaEmUy8cpaKG3mK6ScHeEDvQf8driDoRxuxy4GPAs_W_Dn_DQascQSGDkdUL4cjmsnRrL6xN-NDp-s_RNwN5pxiCo"
        }
    }
}
